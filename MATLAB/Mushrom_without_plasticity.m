...
CS2 = [0,0,0,1,0,0,0,1,0];    % neutral odor 2
...
S_i   = [CS1_i; CS2_i; US1_i; US2_i];   % combine stimulus activities for matrix multiplication

S_0   = zeros(4,1);  % initial values
...
K21_i = nan(1,N_i);  % odor 2, rewarded
K22_i = nan(1,N_i);  % odor 2, punished

K_0 = zeros(4,1);

K_i   = [K11_i; K12_i; K21_i; K22_i];   % combine K-cell activities for matrix multiplication
...
K21_input_i = max( [CS2_i; US1_i; alpha*CS2_i.*US1_i], [], 1 );
K22_input_i = max( [CS2_i; US2_i; alpha*CS2_i.*US2_i], [], 1 );
...
M_matrix = [0 , gamma , beta , 0 , beta , 0 ; gamma , 0 , 0 , beta , 0 , beta];
           %[M1 , M2 , K1_1(reward), K1_2(punish), K2_1, K2_2 ]
...
hold off;
legend('( KC1_1) odour1 + reward','(KC1_2) odour1 + punishment','(KC2_1) odour2 + reward','(KC2_2)odour2 + punishment');
